    ...
    internal class Sound
    {
        ...
        public Sound(string name, string audioFileNameNoExt, SoundType type, float minDistance, float distanceCutoff, bool loop, float volume = 1f)
        {
            lock (SoundLock)
            {
                ...
                DBCID = IDGenerationTool.GenerateID("SoundEntryID", name, audioFileNameNoExt, Convert.ToInt32(type).ToString(),
                    minDistance.ToString(CultureInfo.InvariantCulture), distanceCutoff.ToString(CultureInfo.InvariantCulture),
                    loop == true ? "1" : "0", volume.ToString(CultureInfo.InvariantCulture));
