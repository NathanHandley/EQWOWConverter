    ...
    internal class CreatureAddonSQL : SQLFile
    {
        public override string DeleteRowSQL()
        {
            ...
            if (Configuration.CONFIGONLY_CREATURE_SPAWN_AND_WAYPOINT_DEBUG_MODE == true)
                sb.AppendLine("DELETE FROM creature_addon WHERE `guid` >= " + Configuration.CONFIGONLY_SQL_CREATURE_GUID_DEBUG_LOW.ToString() + " AND `guid` <= " + Configuration.CONFIGONLY_SQL_CREATURE_GUID_DEBUG_HIGH + ";");
            sb.AppendLine("DELETE FROM creature_addon WHERE `guid` >= " + Configuration.SQL_CREATURE_GUID_LOW.ToString() + " AND `guid` <= " + Configuration.SQL_CREATURE_GUID_HIGH + ";");
            sb.Append("DELETE FROM creature_addon WHERE `guid` >= " + Configuration.SQL_CREATURE_GUID_BLOCK2_LOW.ToString() + " AND `guid` <= " + Configuration.SQL_CREATURE_GUID_BLOCK2_HIGH + ";");
            return sb.ToString();
        }
