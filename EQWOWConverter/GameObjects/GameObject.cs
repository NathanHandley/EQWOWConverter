        ...
        private static void LoadGameObjects()
        {
            ...
            foreach (Dictionary<string, string> gameObjectsRow in gameObjectsRows)
            {
                ...
                if (gameObjectType != GameObjectType.NonInteract && gameObjectType != GameObjectType.Emitter)
                {
                    ...
                    if (zonePropertiesByShortName[zoneShortName].ShouldGenerateInstanceDungeon() == true)
                        newGameObject.GameObjectGUIDDungeon = IDGenerationTool.GenerateID("GameObjectGUID", "gameobjectdungeon", newGameObject.ID.ToString());
                }
            ...
            foreach (GameObject gameObject in interactiveGameObjects)
            {
                ...
                if (interactiveGameObjectsByZoneShortNameAndDoorID.ContainsKey((gameObject.ZoneShortName, gameObject.TriggerDoorID)))
                {
                    ...
                    gameObject.TriggerGameObjectGUIDDungeon = interactiveGameObjectsByZoneShortNameAndDoorID[(gameObject.ZoneShortName, gameObject.TriggerDoorID)].GameObjectGUIDDungeon;
            ...
            foreach (var gameObjectByShortName in allNonDoodadGameObjectsByZoneShortName)
            {
                foreach (GameObject gameObject in gameObjectByShortName.Value)
                {
                    ...
                    else
                    {
                        ...
                        int gameObjectDisplayInfoID = IDGenerationTool.GenerateID("GameObjectDisplayInfoID", "gameobject", gameObject.OriginalModelName, gameObject.OpenType.ToString());
