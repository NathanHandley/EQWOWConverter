            ...
            foreach (TransportShip transportShip in transportShips)
            {
                ...
                if (transportShipZoneModelsByShipName.ContainsKey(shipName) == false)
                {
                    ...
                    if (transportShip.IsSkeletal == true)
                    {
                        ...
                        CreatureModelTemplate creatureModelTemplate = new CreatureModelTemplate(transportRace, CreatureGenderType.Male, 0, transportShip.EQTexture, 0, 0, 1f, false, false, false);
...
                    // Note that EQ uses 0-512, so normalize that first
            ...
            foreach (QuestTemplate questTemplate in questTemplates)
            {
                ...
                if (questTemplate.Reactions.Count > 0)
                    foreach (QuestReaction reaction in questTemplate.Reactions)
                        if (reaction.ReactionType == QuestReactionType.Emote || reaction.ReactionType == QuestReactionType.Say || reaction.ReactionType == QuestReactionType.Yell || reaction.ReactionType == QuestReactionType.CastSpell)
                            foreach (CreatureTemplate creatureTemplate in questgiverCreatureTemplates)
        ...
        public void GenerateCustomSpells(ref List<SpellTemplate> spellTemplates)
        {
            ...
            if (Configuration.ACHIEVEMENT_EQ_ADVENTURER_ENABLED == true)
            {
                ...
                eqAdventurerSpellTemplate.AuraDescription = "Gaining experience only through Everquest content. Lost if you complete a non-Everquest quest, or kill a non-Everquest creature that grants experience or reputation.";
            ...
            // Bash
            int bashAndSlamSpellCategoryID = SpellCategoryDBC.GenerateDBCID("bashslam"); // Used for linking cooldown between bash and slam
